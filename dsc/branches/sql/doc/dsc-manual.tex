...
beginning with ``{\tt DBI:Pg:}'' (for PostgreSQL)
or ``{\tt DBI:mysql:}'' (for MySQL).
The value of {\tt dbi\_username} should be ``{\tt dsc}'' (or any other
database user with permission to select, update, insert, delete, and
create tables in the {\tt dsc} schema).  Then run the following
shell command:
